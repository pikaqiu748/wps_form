...
router.get(
  ...
  async (ctx: Context) => {
    try {
      const token = ctx.cookies.get('token')
      const forms = await formKit.getAllFroms()
      ctx.body = generateOk({ forms })
    } catch (err) {
      catchError(err, ctx)
    }
  }
...
router.post(
  ...
  async (ctx: Context) => {
    try {
      ...
      await formKit.starForm(token, id, isStar)
      ctx.body = generateOk()
    } catch (err) {
      catchError(err, ctx)
    }
...
router.post(
  ...
  async (ctx: Context) => {
    try {
      const { id } = ctx.request.body
      const form = await formKit.deleteForm(id)
      ctx.body = generateOk({ form })
    } catch (err) {
      catchError(err, ctx)
    }
  }
...
router.post('/release', async (ctx: Context) => {
  try {
    const { id } = ctx.request.body
    const forms = await formKit.release(id)
    ctx.body = generateOk({ forms })
  } catch (err) {
    catchError(err, ctx)
  }
...
router.post('/stop', async (ctx: Context) => {
  try {
    const { id } = ctx.request.body
    const forms = await formKit.stopCollect(id)
    ctx.body = generateOk({ forms })
  } catch (err) {
    catchError(err, ctx)
  }
...
// 表单取消标星
router.get('/showOnlyStar', async (ctx: Context) => {
  try {
    const forms = await formKit.showOnlyStar()
    ctx.body = generateOk({ forms })
  } catch (err) {
    catchError(err, ctx)
  }
...
router.post(
  ...
  async (ctx: Context) => {
    try {
      const token = ctx.cookies.get('token')
      const { id } = ctx.params
      const item = formResultKit.getFormProblem(token, id)
      ctx.body = generateOk({
        item,
      })
    } catch (err) {
