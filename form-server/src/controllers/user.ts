...
import { IChangeAccount, IChangePwdReq, ISetUserInfoReq } from '../types/request'
...
router.post(
  ...
  async (ctx: Context) => {
    try {
      const { account,newAccount} = ctx.request.body as IChangeAccount
      await userKit.setUserAccount(account,newAccount)
      ctx.body = generateOk()
    } catch (err) {
      catchError(err, ctx)
    }
