...
export function showOnlyStar() {
  const form = db
    .get('forms')
    .filter((form) => form.isStar === true)
    .value()
  return form
}
...
export function deleteForm(id: string) {
  const form = db
    ...
    .assign({ status: EFormStatus.delete })
    .write()
  check(!!form, 'ERR_FORM_NOT_FOUND')
  return form
}
...
export function release(id: string) {
  const form = db
    .get('forms')
    .find((form) => form.id === id)
    .assign({ status: EFormStatus.ing })
    .write()
  check(!!form, 'ERR_FORM_NOT_FOUND')
  return form
}

export function stopCollect(id: string) {
  const form = db
    .get('forms')
    .find((form) => form.id === id)
    .assign({ status: EFormStatus.end })
    .write()
  check(!!form, 'ERR_FORM_NOT_FOUND')
  return form
}
