<template>
  <div ref="codeDom" :class="{ 'form-question': visible, 'one-problem': true }" >
    <div class="input-question">
      <el-form ref="ruleFormRef" :model="newProblem"  :rules="rules">
        <el-form-item prop="title" :label="(index + 1)+'.'">
          <el-input v-model="newProblem.title" autosize type="textarea" maxlength="500" placeholder="请输入问题"
    ...
    <DateQuestion v-else-if="type == 'date' || type == 'time'" :type="type"></DateQuestion>
    <ChoiceQuestion v-else-if="type == 'singleSelect' || type == 'pullSelect' || type == 'multiSelect'"
     :type="type" :visible="visible" :options="newProblem.setting.options">
    </ChoiceQuestion>
    <!-- bottem底部 -->
    <div class="question-bottem" v-show="visible">
      <div class="bottem">
        <el-select v-if="type == 'input' || type == 'date' || type == 'time'" v-model="selectValue" class="m-2"
          placeholder="" size="small" @change="changeType()">
          <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
        <el-select v-else-if="type == 'singleSelect' || type == 'multiSelect' || type == 'pullSelect'"
          v-model="selectValue" class="m-2" placeholder="" size="small" @change="changeType()">
...
//校验
 const validateHandle1 = async (callback:Function) => {
...
//将validateHandle1方法暴露给父组件CreateForm使用
defineExpose({
  validateHandle1
})
</script>
